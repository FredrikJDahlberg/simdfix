...
#include "org/limitless/simdfix/generated/messages/FixMessageDecoders.hpp"
#include "org/limitless/simdfix/generated/messages/FixMessageHandler.hpp"

#define SOH "\x01"

namespace org::limitless::simdfix::decoder {

using namespace org::limitless::simdfix::decoder;
using namespace org::limitless::simdfix::generated::config;
...
TEST(MessageDecoder, LogonWithXmlDataInlineSkip)
{
    ...
    // No DataFields argument: the default, generated from the spec, maps 212 to 213.
    PayloadDecoder<Protocol::FIXT_1_1> decoder;
    const auto message = heap(utils::makeSpan(
        "8=FIXT.1.1" SOH "9=0091" SOH "35=A" SOH "49=SENDER" SOH "56=TARGET" SOH
        "34=1" SOH "52=20260613-19:26:13.959" SOH "98=0" SOH "108=30" SOH
...
// Frames a FIXT.1.1 message around body: prepends BeginString and BodyLength,
// appends the CheckSum.
[[nodiscard]] std::vector<uint8_t> frame(const std::string& body)
{
    std::string message = "8=FIXT.1.1" SOH "9=" + std::to_string(body.size()) + SOH + body;
    uint32_t sum = 0;
    ...
    message += "10=" + std::string(3 - checkSum.size(), '0') + checkSum + SOH;
    return {message.begin(), message.end()};
}

// Group1(1000) entries each carry their own NestedGroup(2000). Every nested
// group must be read from its own outer entry, the outer entry's fields must
// still resolve while and after its nested group is iterated, and once
// iteration ends the message-level fields must resolve against the whole
// message again.
TEST(MessageDecoder, NestedGroups)
{
    struct AppHandler : FixMessageHandler<AppHandler>
    {
        using FixMessageHandler::handle;
        ...
        Result handle(ExecutionReportDecoder& report)
        {
            ...
            {
                auto& outer = report.group1();
                EXPECT_EQ(2U, outer.count());
                while (outer.hasNext())
                {
                    outer.next();
                    const auto compID = toString(outer.group1CompID().value_or(std::string_view{}));
                    read.push_back(compID + ":" + std::to_string(outer.group1RefID().value_or(0)));
                    auto& inner = outer.nestedGroup();
                    read.push_back(compID + ".count=" + std::to_string(inner.count()));
                    while (inner.hasNext())
                    {
                        inner.next();
                        const auto name = toString(inner.name().value_or(std::string_view{}));
                        // An outer field between two inner ones, in the same inner entry.
                        const auto outerRefID = outer.group1RefID().value_or(0);
                        read.push_back(name + ":" + std::to_string(inner.nestedOne().value_or(0)) +
                                       " in " + std::to_string(outerRefID));
                    }
                    read.push_back(compID + " after:" + std::to_string(outer.group1RefID().value_or(0)));
                }
                EXPECT_EQ("MSFT", toString(report.symbol().value_or(std::string_view{})));
                EXPECT_EQ(200U, report.leavesQty().value_or(0));
            }
            found = true;
            return Result::Success;
        }
    ...
    const auto message = frame(
        "35=8" SOH "49=SENDER" SOH "56=TARGET" SOH "34=1" SOH "52=20260613-19:26:13.959" SOH
        "37=ORD002" SOH "17=EXEC002" SOH "150=0" SOH "39=0" SOH "55=MSFT" SOH "54=2" SOH
        "151=200" SOH "14=0" SOH "6=0" SOH "60=20260613-19:26:13.959" SOH
        "1000=2" SOH
            "1001=A" SOH "1003=1" SOH
            "2000=2" SOH
                "2001=A1" SOH "601=11" SOH
                "2001=A2" SOH "601=12" SOH
            "1001=B" SOH "1003=2" SOH
            "2000=1" SOH
                "2001=B1" SOH "601=21" SOH);
    ...
    const std::vector<std::string> onePass{
        "A:1", "A.count=2", "A1:11 in 1", "A2:12 in 1", "A after:1",
        "B:2", "B.count=1", "B1:21 in 2", "B after:2"};
    std::vector<std::string> expected{onePass};
    expected.insert(expected.end(), onePass.begin(), onePass.end());
    EXPECT_EQ(expected, app.read);
}
...
TEST(MessageDecoder, InvalidBeginString)
{
    ...
    // "FIX.4.2" does not match the PayloadDecoder<Protocol::FIXT_1_1> expected prefix
    const auto message = utils::makeSpan(
