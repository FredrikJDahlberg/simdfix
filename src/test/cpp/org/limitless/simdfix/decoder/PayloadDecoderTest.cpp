...
#include "org/limitless/simdfix/generated/messages/FixTypes.hpp"
...
// Parses a Logon carrying payload as XmlData(213), with a SenderCompID of pad
// characters to move it within the 16-byte blocks, and checks the tokens are
// what data skipping yields.
static void expectDataFieldSkipped(const std::string& payload, const size_t pad)
{
    const auto message = build({"35=A", "49=" + std::string(pad, 'S'), "56=T", "34=1",
        "52=20260613-19:26:13.959", "98=0", "108=30",
        "212=" + std::to_string(payload.size()), "213=" + payload, "58=after"});
    const std::vector<uint8_t> buffer(message.begin(), message.end());

    PayloadDecoder<Protocol::FIXT_1_1> decoder;
    const auto [processed, status] = decoder.parse(Buffer{buffer.data(), buffer.size()});
    ASSERT_EQ(Result::Success, status) << "pad " << pad;
    ASSERT_EQ(buffer.size(), processed) << "pad " << pad;

    std::vector<uint16_t> tags;
    for (const auto& field : decoder.fields())
    {
        tags.push_back(field.m_tag);
        if (field.m_tag == 213)
        {
            EXPECT_EQ(payload.size(), static_cast<size_t>(field.m_length)) << "pad " << pad;
            EXPECT_EQ(payload, std::string(reinterpret_cast<const char*>(buffer.data()) + field.m_position,
                                           field.m_length)) << "pad " << pad;
        }
    }
    const std::vector<uint16_t> expected{8, 9, 35, 49, 56, 34, 52, 98, 108, 212, 213, 58, 10};
    EXPECT_EQ(expected, tags) << "pad " << pad;
}

// The default decoder scans without data skipping, then drops the tokens made
// from payload bytes, or rescans with data skipping if the payload ended that
// scan early. Shifting the message a byte at a time puts the length tag, its
// value and the payload at every position within a 16-byte block.
TEST(PayloadDecoder, DataFieldAtEveryBlockAlignment)
{
    const std::vector<std::string> payloads{
        "<a" SOH "35=X" SOH "58=Y" SOH "b=1>",        // tokens to drop
        "<a" SOH "35=X" SOH "10=000" SOH "b=1>",      // ends the first scan early: rescanned
        "x" SOH "12",                                 // digits ending the payload
        "",
    };
    for (const auto& payload : payloads)
    {
        for (size_t pad = 1; pad <= 2 * Uint8x16::Size; ++pad)
        {
            expectDataFieldSkipped(payload, pad);
        }
    }
}

// Random payloads made of the bytes that shape tokens.
TEST(PayloadDecoder, DataFieldRandomPayloads)
{
    constexpr std::string_view Alphabet = SOH SOH SOH "==0123456789ab";
    std::mt19937 random{12345};
    for (int iteration = 0; iteration < 4000; ++iteration)
    {
        std::string payload(random() % 48, ' ');
        for (auto& byte : payload)
        {
            byte = Alphabet[random() % Alphabet.size()];
        }
        expectDataFieldSkipped(payload, 1 + random() % Uint8x16::Size);
        if (HasFailure())
        {
            return;
        }
