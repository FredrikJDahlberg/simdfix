...
#include "org/limitless/generator/simdfix/DataModel.hpp"

using namespace org::limitless::simdfix;
using namespace org::limitless::generator::simdfix;

// C++ namespace the generated code lives in, set with --namespace. Its path form
// is the include prefix the generated headers use for each other, and its upper-
// case form prefixes their include guards, so two specs generated into different
// namespaces can be used side by side.
static std::string g_namespace{"org::limitless::simdfix::generated"};

static std::string replaceAll(std::string text, const std::string_view from, const std::string_view to)
{
...
static bool isValidNamespace(const std::string_view ns)
{
    if (ns.empty() || ns.starts_with("::") || ns.ends_with("::"))
    {
        return false;
    }
    const auto parts = replaceAll(std::string{ns}, "::", " ");
    bool start = true;
    for (const char ch : parts)
    {
        if (ch == ' ')
        {
            start = true;
        }
        else if (std::isalpha(static_cast<unsigned char>(ch)) || ch == '_' || (!start && std::isdigit(static_cast<unsigned char>(ch))))
        {
            start = false;
        }
        else
    ...
    return parts.find("  ") == std::string::npos;
}

// "<namespace>::<part>", e.g. org::limitless::simdfix::generated::messages
static std::string generatedNamespace(const std::string_view part)
{
    return std::format("{}::{}", g_namespace, part);
}

// #include line for another generated header, e.g. "org/limitless/simdfix/generated/messages/FixTypes.hpp"
static std::string generatedInclude(const std::string_view part, const std::string_view file)
{
    return std::format("#include \"{}/{}/{}\"\n", replaceAll(g_namespace, "::", "/"), part, file);
}

// Include guard for a generated header, e.g. ORG_LIMITLESS_SIMDFIX_GENERATED_MESSAGES_FIX_TYPES_HPP
static std::string generatedGuard(const std::string_view part, const std::string_view name)
{
    std::string guard = std::format("{}_{}_{}_HPP", replaceAll(g_namespace, "::", "_"), part, name);
    std::ranges::transform(guard, guard.begin(), [](const unsigned char ch) { return std::toupper(ch); });
    return guard;
}

static void openGenerated(std::ostream& out, const std::string_view part, const std::string_view name)
{
    const auto guard = generatedGuard(part, name);
    out << std::format("#ifndef {}\n#define {}\n\n", guard, guard);
}

static void openNamespace(std::ostream& out, const std::string_view part)
{
    out << std::format("namespace {} {{\n\n", generatedNamespace(part));
    // The generated namespace need not be nested in simdfix's, so make its names visible.
    out << "using namespace ::org::limitless::simdfix;\n\n";
}

static void closeGenerated(std::ostream& out, const std::string_view part, const std::string_view name)
{
    out << std::format("}} // namespace {}\n\n", generatedNamespace(part));
    out << std::format("#endif //{}\n", generatedGuard(part, name));
}

static void generateUmbrella(const std::string& fileName);
...
static void generateEngineConfig(const std::string& fileName,
                                 ...
                                 const std::map<int32_t, int32_t>& dataFields);

// Collects every (lengthTag -> dataTag) pair from <data> elements across all
// messages, for PayloadDecoder's tokenizing-time binary-safe data-field skip.
// A length tag claimed by two different data tags is a schema inconsistency,
// not a case to silently resolve, so it hard-fails generation.
static std::map<int32_t, int32_t> collectDataFields(const std::vector<Record>& records)
{
    std::map<int32_t, int32_t> dataFields;
    for (const auto& record : records)
    {
        for (const auto& field : record.m_records)
        {
            if (field.m_category != Category::Data)
            {
                continue;
            }
            const int32_t lengthTag = field.m_length;
            const int32_t dataTag = field.m_tag;
            auto [it, inserted] = dataFields.try_emplace(lengthTag, dataTag);
            if (!inserted && it->second != dataTag)
            {
                std::println("Error: length tag {} maps to both data tag {} and {}",
                             lengthTag, it->second, dataTag);
                std::exit(1);
            }
        }
    }
...
{
    // --namespace <ns> may appear anywhere; the rest are positional.
    std::vector<char*> args{argv[0]};
    for (int i = 1; i < argc; ++i)
    {
        if (std::string_view{argv[i]} == "--namespace" && i + 1 < argc)
        {
            g_namespace = argv[++i];
        }
        else
    ...
    if (argc < 5)
    {
        std::println("Usage: generator [--namespace <ns>] <session file> <output directory> <config file> "
                     "<config output directory> [<application file>]");
        return 1;
    }
    if (!isValidNamespace(g_namespace))
    {
        std::println("Error: '{}' is not a valid C++ namespace", g_namespace);
        return 1;
    }
    ...
    std::string umbrellaFile{argv[2]};
    umbrellaFile.append("/FixMessages.hpp");
    generateUmbrella(umbrellaFile);
    ...
    generateEngineConfig(engineFile, config, collectDataFields(model.m_records));
                          ...
                          const std::vector<Record>& enums)
{
    ...
    openGenerated(out, "messages", "FixTypes");
    out << "#include <cstdint>\n";
    out << "#include <string_view>\n\n";
    out << "#include \"org/limitless/simdfix/Types.hpp\"\n\n";
    openNamespace(out, "messages");
    for (const auto& type: enums)
    {
        generateEnums(out, type);
    }
    closeGenerated(out, "messages", "FixTypes");
                             ...
                             const std::vector<Record>& records)
{
    ...
    out << "// Generated by Generator. Do not edit by hand.\n";
    openGenerated(out, "messages", "FixMessageDecoders");
    out << "#include \"org/limitless/simdfix/detail/Expected.hpp\"\n\n";
    out << generatedInclude("config", "FixEngine.hpp");
    out << generatedInclude("messages", "FixTypes.hpp");
    out << "#include \"org/limitless/simdfix/decoder/DataDecoder.hpp\"\n";
    out << "#include \"org/limitless/simdfix/decoder/GroupDecoder.hpp\"\n";
    out << "#include \"org/limitless/simdfix/decoder/MessageDecoder.hpp\"\n";
    out << "#include \"org/limitless/simdfix/decoder/PayloadDecoder.hpp\"\n\n";
    openNamespace(out, "messages");
    out << "using namespace ::org::limitless::simdfix::decoder;\n\n";
    out << "// The decoder for this spec: its field capacity and data-field pairs come from its config.\n";
    out << "template <Protocol P, typename DataFields = ::" << generatedNamespace("config") << "::DataFields>\n";
    out << "using PayloadDecoder = BasicPayloadDecoder<P, ::" << generatedNamespace("config")
        << "::MaxFields, DataFields>;\n\n";
    for (auto& record: records)
    {
        generateRecordDecoders(out, record);
    }
    closeGenerated(out, "messages", "FixMessageDecoders");
                             ...
                             const std::vector<Record>& records)
{
    ...
    out << "// Generated by Generator. Do not edit by hand.\n";
    openGenerated(out, "messages", "FixMessageEncoders");
    out << "#include <string>\n\n";
    out << generatedInclude("config", "FixEngine.hpp");
    out << generatedInclude("messages", "FixTypes.hpp");
    out << "#include \"org/limitless/simdfix/encoder/DataEncoder.hpp\"\n";
    out << "#include \"org/limitless/simdfix/encoder/GroupEncoder.hpp\"\n";
    out << "#include \"org/limitless/simdfix/encoder/MessageEncoder.hpp\"\n";
    out << "#include \"org/limitless/simdfix/encoder/PayloadEncoder.hpp\"\n";
    out << "#include \"org/limitless/simdfix/encoder/ResendEncoder.hpp\"\n\n";
    openNamespace(out, "messages");
    out << "using namespace ::org::limitless::simdfix::encoder;\n\n";
    out << "// PossDup resend for this spec, decoding with its field capacity and data-field pairs.\n";
    out << "template <Protocol P, typename DataFields = ::" << generatedNamespace("config") << "::DataFields>\n";
    out << "[[nodiscard]] std::size_t encodeResend(const Buffer original, const std::span<uint8_t> out,\n";
    out << "                                        const std::chrono::milliseconds currentSendingTime)\n";
    out << "{\n";
    out << "    return basicEncodeResend<P, ::" << generatedNamespace("config")
        << "::MaxFields, DataFields>(original, out, currentSendingTime);\n";
    out << "}\n\n";
    for (auto& record: records)
    {
    ...
    out << "};\n\n";
    closeGenerated(out, "messages", "FixMessageEncoders");
                                 ...
                                 const std::map<int32_t, int32_t>& dataFields)
{
    ...
    openGenerated(out, "config", "FixEngine");
    out << "#include <chrono>\n";
    out << "#include <cstdint>\n";
    out << "#include <string_view>\n\n";
    out << "#include \"org/limitless/simdfix/Types.hpp\"\n\n";

    openNamespace(out, "config");
    ...
    // PayloadDecoder's tokenizing-time data-field skip: maps a length tag to
    // its paired data tag so embedded SOH/'=' bytes in the data's raw value
    // aren't mistaken for field framing. Empty schema degenerates to the
    // default case only, equivalent to NoDataFields.
    out << "struct DataFields\n{\n";
    out << "    static constexpr int32_t dataTag(const uint16_t tag)\n    {\n";
    out << "        switch (tag)\n        {\n";
    for (const auto& [lengthTag, dataTag] : dataFields)
    {
        out << std::format("            case {}: return {};\n", lengthTag, dataTag);
    }
    ...
    closeGenerated(out, "config", "FixEngine");
}
...
static void generateMessageHandler(const std::string& fileName, const std::vector<Record>& records)
{
    ...
    out << "// Generated by Generator. Do not edit by hand.\n";
    openGenerated(out, "messages", "FixMessageHandler");
    out << generatedInclude("messages", "FixMessageDecoders.hpp") << "\n";
    openNamespace(out, "messages");
    ...
    out << "    }\n";
    out << "};\n\n";
    closeGenerated(out, "messages", "FixMessageHandler");
}

/**
 * Emits FixMessages.hpp, the one header a consumer includes: simdfix's
 * hand-written API plus everything generated for this spec.
 */
static void generateUmbrella(const std::string& fileName)
{
    std::ofstream out(fileName);
    ...
    out << "// Generated by Generator. Do not edit by hand.\n";
    openGenerated(out, "messages", "FixMessages");
    out << "#include \"org/limitless/simdfix/Fix.hpp\"\n";
    out << generatedInclude("config", "FixEngine.hpp");
    out << generatedInclude("messages", "FixTypes.hpp");
    out << generatedInclude("messages", "FixMessageDecoders.hpp");
    out << generatedInclude("messages", "FixMessageEncoders.hpp");
    out << generatedInclude("messages", "FixMessageHandler.hpp") << "\n";
    out << std::format("#endif //{}\n", generatedGuard("messages", "FixMessages"));
}
...
static bool isCacheableField(const org::limitless::generator::simdfix::Field& field)
{
...
static bool isRequiredCacheableField(const org::limitless::generator::simdfix::Field& field)
{
    return field.m_presence == Presence::Required && isCacheableField(field);
}

static std::string fieldReturnType(const org::limitless::generator::simdfix::Field& field)
{
...
static std::string decoderCall(const org::limitless::generator::simdfix::Field& field, const Record& record)
{
    ...
    // A group's fields are read from its own current entry, not whichever scope is innermost.
    const auto depth = record.m_parent == RecordType::Group ? "scopeDepth()" : "";
    return std::format("m_decoder.get{}<{}, {}{}, RecordType::{}>({})",
                       name(field.m_category), field.m_tag, mandatory, arg, parent, depth);
}
...
static void generateWrapNextDecoders(std::ostream& out, const Record& record)
{
    ...
    {
        out << std::format("    {}Decoder& wrap(const int32_t depth = 0)\n", record.m_name);
        out << "    {\n";
        out << std::format("        GroupDecoder::wrap<{}>(depth);\n", record.m_tag);
        out << "        return *this;\n";
        out << "    }\n\n";
...
static void generateGetters(std::ostream& out, const Record& record)
{
    ...
    for (const auto& comp: record.m_records)
    {
        ...
        else
        {
            ...
            if (comp.m_tag != 0)
            {
                // A group nested in a group is scoped to the enclosing entry.
                out << (record.m_parent == RecordType::Group ? ".wrap(childDepth())" : ".wrap()");
            }
            out << ";\n";
            out << std::format("    }}\n\n");
        }
