...
#include "org/limitless/generator/fix/ConfigMapping.hpp"
                             ...
                             const std::vector<Record>& records)
{
    ...
    out << "#include \"org/limitless/fix/generated/config/FixEngine.hpp\"\n";
    out << "#include \"org/limitless/fix/generated/messages/FixTypes.hpp\"\n";
                                 ...
                                 const Record& protocol)
{
    ...
    out << "namespace org::limitless::fix::generated::config {\n\n";
...
static void generateConfig(const std::string& fileName, const pugi::xml_document& config)
{
    ...
    for (const auto session : root.child("sessions").children("session"))
    {
        ...
        for (const auto& header : {storage.header, transport.header, application.header})
        {
            if (!header.empty())
            {
