...
int main(int argc, char** argv)
{
    ...
    if ((argc != 3 && argc != 4) || !result)
    {
        std::println("XML error: {}, dir = {}, file = {}", result.description(),
                     std::filesystem::current_path().c_str(), argv[1]);
        std::println("Usage: generator <protocol file> <output directory> [config file]");
        return 1;
    }
...
// Maps a config storage attribute to a C++ type: the known library strategies,
// or a fully-qualified custom type written verbatim by the author.
static std::string storageType(const std::string& value)
{
    if (value.empty() || value == "null") return "storage::NullStorage";
    if (value == "memory") return "storage::MemoryStorage";
    return value;
}
...
static void generateConfig(const std::string& fileName, const pugi::xml_document& config)
{
    ...
    out << "#include <variant>\n\n";
    out << "#include \"org/limitless/fix/messages/FixEngine.hpp\"\n";
    out << "#include \"org/limitless/fix/session/ClientSession.hpp\"\n";
    out << "#include \"org/limitless/fix/session/ServerSession.hpp\"\n";
    out << "#include \"org/limitless/fix/storage/MemoryStorage.hpp\"\n\n";
    out << "namespace org::limitless::fix::config {\n\n";
    out << "using namespace org::limitless::fix::session;\n\n";
    ...
    for (const auto session : root.child("sessions").children("session"))
    {
        ...
        const std::string storage = storageType(session.attribute("storage").as_string());
        const std::string transport = session.attribute("transport").as_string("DiscardTransport");
        const std::string application = session.attribute("application").as_string("RejectApplication");
        const auto* klass = role == "client" ? "ClientSession" : "ServerSession";
        const std::string alias = cap(name) + "Session";

        out << std::format("using {} = {}<{}, \"{}\", \"{}\", {}, {}, {}>;\n",
                           alias, klass, beginString, compId, remote, storage, transport, application);
        sessions.push_back({name, alias, storage, session.attribute("heartbeat").as_string()});
    }
