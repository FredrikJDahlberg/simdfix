...
#include "org/limitless/simdfix/detail/encoder/FieldEncoder.hpp"
#include "org/limitless/simdfix/utils/Conversions.hpp"

namespace org::limitless::simdfix::encoder
{
 ...
 * Each generated spec provides an encodeResend<Protocol> wrapper that fills in
 * MaxFields and DataFields from its config.
 * @tparam Protocol FIX protocol version (BeginString), as for BasicPayloadDecoder
 * @tparam MaxFields token capacity, as for BasicPayloadDecoder
 * @tparam DataFields binary-safe data-field schema, as for BasicPayloadDecoder
 ...
 *         or `out` is too small
 */
template <auto Protocol, std::size_t MaxFields, typename DataFields>
[[nodiscard]] std::size_t basicEncodeResend(const Buffer original, const std::span<uint8_t> out,
                                             const std::chrono::milliseconds currentSendingTime)
{
    ...
    decoder::BasicPayloadDecoder<Protocol, MaxFields, DataFields> resendDecoder;
