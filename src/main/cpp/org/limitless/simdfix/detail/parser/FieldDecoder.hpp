...
#include <array>
#include "org/limitless/simdfix/detail/Expected.hpp"
#include <span>
#include <chrono>
#include <limits>
#include <utility>
...
#include "org/limitless/simdfix/detail/simd/LinearSearch.hpp"
#include "org/limitless/simdfix/utils/Conversions.hpp"

namespace org::limitless::simdfix::detail::parser {
...
public:
     ...
     * with ParentType::Group to [begin, end) until truncateGroupScopes leaves it.
     * @param begin first token/tag index of the group entry
     * @param end exclusive last token/tag index of the group entry
     * @return false, with no scope entered, if group nesting would exceed MaxGroupDepth
     */
    [[nodiscard]] bool pushGroupScope(const int32_t begin, const int32_t end)
    {
    ...
    /**
     * Leaves every repeating-group scope at or below the given nesting depth,
     * keeping the depth scopes that enclose it. A depth at or beyond the
     * current one leaves the scopes unchanged.
     * @param depth number of enclosing scopes to keep
     */
    void truncateGroupScopes(const int32_t depth)
    {
        if (depth < m_scopeDepth)
        {
            m_scopeDepth = depth < 0 ? 0 : depth;
        }
    }

    /**
     * @param depth nesting depth of a group, as passed to GroupDecoder::wrap
     * @return the scope of that group's current entry, or an empty scope if the
     *         group has no current entry
     */
    [[nodiscard]] Scope groupScopeAt(const int32_t depth) const
    {
        return depth >= 0 && depth < m_scopeDepth ? m_scopes[depth] : Scope{0, 0};
    }

    /**
     * SIMD search for Tag, restricted to a group entry's scope when Parent is
     * ParentType::Group, otherwise across the whole message.
     * @tparam Tag tag number to find
     * @tparam Parent context the tag is being looked up in
     * @param depth for ParentType::Group, the nesting depth of the group whose
     *        current entry to search; -1 searches the innermost active scope
     * @return token/tag index, or -1 if not found
     */
    template <int32_t Tag, RecordType Parent>
    [[nodiscard]] int32_t findIndex(const int32_t depth = -1) const
    {
        if constexpr (Parent == RecordType::Group)
        {
            const auto [begin, end] = depth < 0 ? groupScope() : groupScopeAt(depth);
     ...
     * @tparam Parent context the tag is being looked up in
     * @param depth for ParentType::Group, the nesting depth of the group whose
     *        current entry to read; -1 reads the innermost active scope
     * @return field value, or Result::RequiredFieldMissing/Success if absent
     */
    template <int32_t Tag, bool Required, RecordType Parent>
    [[nodiscard]] constexpr StringResult getString(const int32_t depth = -1) const
    {
        const auto index = findIndex<Tag, Parent>(depth);
        if (index >= 0)
        {
            const auto& field = m_fields[index];
     ...
     */
    template <int32_t Tag, bool Required, RecordType Parent>
    [[nodiscard]] Uint32Result getUint32(const int32_t depth = -1) const
    {
        const auto index = findIndex<Tag, Parent>(depth);
        if (index >= 0)
        {
     ...
     */
    template <int32_t Tag, bool Required, RecordType Parent>
    [[nodiscard]] Int32Result getInt32(const int32_t depth = -1) const
    {
        const auto index = findIndex<Tag, Parent>(depth);
        if (index >= 0)
        {
     ...
     */
    template <int32_t Tag, bool Required, RecordType Parent>
    [[nodiscard]] FixedDecimalResult getFixedDecimal(const int32_t depth = -1) const
    {
        const auto index = findIndex<Tag, Parent>(depth);
        if (index >= 0)
        {
    ...
    [[nodiscard]] constexpr TimestampResult getTimestamp(const int32_t depth = -1) const
    {
        const auto index = findIndex<Tag, Parent>(depth);
        if (index >= 0)
        {
     ...
     */
    template <uint32_t Tag, bool Required, RecordType Parent>
    [[nodiscard]] constexpr TimestampResult getUTCTimeOnly(const int32_t depth = -1) const
    {
        const auto index = findIndex<Tag, Parent>(depth);
        if (index >= 0)
        {
     ...
     */
    template <uint32_t Tag, bool Required, RecordType Parent>
    [[nodiscard]] constexpr TimestampResult getUTCDateOnly(const int32_t depth = -1) const
    {
        const auto index = findIndex<Tag, Parent>(depth);
        if (index >= 0)
        {
    ...
    [[nodiscard]] constexpr expected<Enum, Result> getEnum(const int32_t depth = -1) const
    {
        const auto index = findIndex<Tag, Parent>(depth);
        if (index >= 0)
        {
